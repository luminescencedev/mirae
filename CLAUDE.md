...
- **Deferred tickets have a home — fold-in, don't lose.** When a ticket is deferred out of its sprint, mark it (`[ ] … (deferred)` or `(→ Sprint N)`) and, if it belongs to a themed sprint (mobile → 18, security/a11y → 22, SEO/analytics → 19), that sprint **absorbs** it when it runs. Genuine polish with no themed home accumulates in a periodic **"deferred polish" sprint** (e.g. Sprint 17.5) that burns the debt down before advancing. Never silently drop a deferred ticket.
